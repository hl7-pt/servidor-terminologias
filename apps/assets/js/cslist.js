...
function initializeDataTable() {
  ...
  t = $('#prod-table').DataTable({
    language: {
      ...
      infoEmpty: 'Sem CodeSystems para mostrar',
      ...
      zeroRecords: 'Nenhum CodeSystem encontrado'
    }
  });
}
...
async function processData(data, baseurl) {
  ...
  for (var i = 0; i < totalCount; i++) {
    ...
    if (resource["resourceType"] == "CodeSystem") {
      ...
      current_row.push(resource.count != null ? resource.count : '-');
      current_row.push(
        '<a href="' + baseurl + '/CodeSystem/' + data[i].id + '">Ver</a>');
      current_row.push(
        '<a href="./visualiser/viz-index.html?url=' + baseurl + '/CodeSystem/' + data[i].id + '">Ver</a>');

      t.row.add(current_row);

      console.log(current_row);
      if (progressIndicator) {
        progressIndicator.innerText = 'A processar CodeSystem ' + (i + 1) + ' de ' + totalCount + '...';
      }
    }
  }
